...
const targetStr = `                  <div className="mt-20">
...
const searchIndex = code.indexOf(targetStr);
if (searchIndex === -1) {
  console.log("Could not find targetStr");
  process.exit(1);
}

const correctEnd = `                  <div className="mt-20">
                    <p className="mb-16">I hereby certify that the information I have provided is true and correct to the best of my knowledge. I understand that this information will be used solely for student profiling.</p>
                    <div className="w-80">
                      <div className="border-b border-black text-center h-6 font-bold uppercase">{selectedSubmission.studentName}</div>
                      <div className="text-center">Signature over Printed Name</div>
...
// Cut off the end of the file starting at searchIndex, and replace it with the exact structure.
const everythingBefore = code.substring(0, searchIndex);
const everythingAfter = code.substring(searchIndex + targetStr.length); // Should just be the `{/* Modal */}` and its contents

// Now we need to make sure everythingAfter ends with `      </div>\n    </div>\n  );\n}` properly.
// Let's just hardcode the modal to make it perfectly safe.
...
fs.writeFileSync('src/pages/guidance/index.tsx', everythingBefore + correctEnd + theModal);
