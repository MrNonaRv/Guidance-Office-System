    ...
    <div className="space-y-8 h-full flex flex-col relative pb-6">
        ...
        {activeTab === 'Form' && (
          <div className="flex flex-col flex-1 relative">
            <div className="flex justify-between items-center mb-8">
              ...
              <button className="bg-[#12306b] hover:bg-[#0a2044] text-white px-8 py-2 rounded-full font-semibold flex items-center gap-2 text-sm transition-colors shadow-sm">
                <Plus className="w-5 h-5 font-bold" /> Add
              </button>
            </div>
            
            <div className="bg-white rounded-md border border-gray-400 p-8 flex items-center shadow-sm">
              <h3 className="text-[#0f2e60] font-black text-[18px] tracking-wide uppercase">STUDENT DEMOGRAPHICS</h3>
            </div>
            
            {/* The space will grow if flex-1 is used, but we might want the button absolute to bottom right of container */}
          </div>
        )}
      </div>
      ...
      {activeTab === 'Form' && (
        <div className="absolute bottom-6 right-6">
          <button className="bg-[#244280] hover:bg-[#1c3566] text-white px-10 py-3 rounded-xl font-bold text-sm shadow-md transition-colors">
            Next
          </button>
        </div>
      )}
