...
// The original print output was basically styled with `print:` classes embedded directly in the main UI
// We want to completely separate the UI view and the Print view.

const startUIIdx = content.indexOf('{/* Form Data Rendering */}');
const endUIIdx = content.indexOf('{/* Modal */}');

if (startUIIdx === -1) throw new Error('Missing Form Data Rendering');

// We will inject a distinct print view right after the normal UI
...
content = content.substring(0, startUIIdx) + printBlock + content.substring(endUIIdx);
