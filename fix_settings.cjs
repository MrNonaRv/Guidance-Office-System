...
content = content.replace(importRegex, (match, p1) => {
  ...
  const newImports = ['Calendar', 'GraduationCap', 'Users', 'FileText', 'Image', 'Plus'].filter(i => !imports.includes(i));
  return `import { ${[...imports, ...newImports].join(', ')} } from 'lucide-react';`;
});

// Replace GuidanceSettings
