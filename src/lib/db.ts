...
export const db = {
  ...
  submissions: {
    ...
    async ensureForStudent(user: { id?: string; email?: string; firstName?: string; lastName?: string }): Promise<Submission | null> {
      if (!user || (!user.email && !user.id)) return null;

      const email = (user.email || '').trim().toLowerCase();
      const id = (user.id || '').trim();
      const fullName = `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Student Applicant';

      // Check current in-memory cache or storage
      const existing = memorySubmissions.find(s => 
        (email && s.studentId && s.studentId.toLowerCase() === email) ||
        (id && s.studentId === id) ||
        (email && s.data?.email && s.data.email.toLowerCase() === email) ||
        (fullName && s.studentName && s.studentName.toLowerCase() === fullName.toLowerCase())
      );
      ...
      // Automatically create starter scholarship application for this student account
      const subId = `SUB-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`;
      const starterSub: Submission = {
        id: subId,
        studentId: email || id,
        ...
        data: {
          studentId: email || `2026-CAPSU-${Math.floor(1000 + Math.random() * 9000)}`,
          ...
          email: email || '',
          permanentAddress: 'Capiz, Philippines',
          scholarshipType: 'Externally-Funded (CHED - Tulong Dunong)',
          scholarshipCategory: 'Tulong Dunong',
          fundingType: 'Externally-Funded',
          academicYear: 'A.Y. 2026-2027 - 1st Semester',
          status: 'Incomplete',
          submittedAt: new Date().toISOString(),
          fatherName: '',
          fatherOccupation: '',
          ...
          guardianOccupation: '',
          guardianContact: '',
          parentEduAttainment: 'High school Graduate',
          monthlyIncome: 'below Php10,000',
          firstInFamily: 'Yes',
          livingWith: 'Parents/Guardians',
          housingType: 'Own house',
          accessToResources: ['Study space', 'Textbooks and learning materials'],
          workingStudent: 'No',
          studentClassification: ['Low income family/ Economically disadvantaged student'],
          workTypeIncome: '',
          specialNeedsCondition: '',
          pdlReason: '',
          scholarshipFundType: 'External',
          internalCategory: '',
          externalCategory: 'CHED Tulong Dunong Program (TDP)',
          chedSubCategory: 'Tulong Dunong Program (TDP)',
          photo2x2: dummyBase64Photo2x2,
          studentIdCard: dummyBase64StudentId,
          signature: dummyBase64Signature,
          signedAt: new Date().toISOString(),
          requirements: [
            { name: '2x2 Recent Formal ID Photo', status: 'Pending' },
            { name: 'Valid Student ID', status: 'Pending' },
            { name: 'Certificate of Grades (COG)', status: 'Pending' },
            { name: 'Certificate of Registration (COR)', status: 'Pending' },
            { name: 'Proof of Income / Certificate of Indigency', status: 'Pending' },
            { name: 'Certificate of Good Moral Character', status: 'Pending' }
          ]
        },
        files: [
          { id: `f-photo-${Date.now()}`, name: `${user.lastName || 'Student'}_2x2_Photo.png`, category: '2x2 Recent Formal ID Photo', type: 'image/png', size: '320 KB', data: dummyBase64Photo2x2, verified: false, status: 'Pending', uploadedAt: new Date().toISOString() },
          { id: `f-id-${Date.now()}`, name: `${user.lastName || 'Student'}_ID.png`, category: 'ID', type: 'image/png', size: '450 KB', data: dummyBase64StudentId, verified: false, status: 'Pending', uploadedAt: new Date().toISOString() },
          { id: `f-rf-${Date.now()}`, name: `${user.lastName || 'Student'}_Registration_Form.pdf`, category: 'RF', type: 'application/pdf', size: '850 KB', data: dummyBase64Pdf, verified: false, status: 'Pending', uploadedAt: new Date().toISOString() },
          { id: `f-gwa-${Date.now()}`, name: `${user.lastName || 'Student'}_GWA_Grades.pdf`, category: 'GWA', type: 'application/pdf', size: '1.2 MB', data: dummyBase64Pdf, verified: false, status: 'Pending', uploadedAt: new Date().toISOString() }
        ]
      };

      await this.create(starterSub);
      return starterSub;
    }
  },
...
export const ensureStudentSubmission = db.submissions.ensureForStudent.bind(db.submissions);
