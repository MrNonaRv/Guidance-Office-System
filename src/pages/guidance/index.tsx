...
export function GuidanceLogin() {
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uid = fbUser ? fbUser.uid : 'admin-guidance-staff';
      let adminUser = await db.users.get(uid);
      if (!adminUser && fbUser?.email) {
        adminUser = await db.users.findByEmail(fbUser.email);
      }
      if (!adminUser) {
        adminUser = {
          id: uid,
        ...
        await db.users.set(uid, adminUser);
      }

      localStorage.setItem('adminAuth', 'true');
      localStorage.setItem('adminEmail', cleanEmail || 'guidancestaff@capsu.edu');
      navigate('/admin/dashboard');
    } catch (err: any) {
      console.error("Admin login error:", err);
