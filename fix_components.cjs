...
// Replace GuidanceCommunications
const commStartStr = 'export function GuidanceCommunications() {';
const commStart = content.indexOf(commStartStr);
let braceCount = 0;
let commEnd = -1;
let inFunction = false;

if (commStart !== -1) {
  for (let i = commStart; i < content.length; i++) {
    if (content[i] === '{') {
      braceCount++;
      inFunction = true;
    } else if (content[i] === '}') {
...
const newComm = `export function GuidanceCommunications() {
...
content = content.substring(0, commStart) + newComm + content.substring(commEnd + 1);

// Replace GuidanceReports
const repStartStr = 'export function GuidanceReports() {';
const repStart = content.indexOf(repStartStr);
let repBraceCount = 0;
let repEnd = -1;
let repInFunction = false;

if (repStart !== -1) {
  for (let i = repStart; i < content.length; i++) {
    if (content[i] === '{') {
      repBraceCount++;
      repInFunction = true;
    } else if (content[i] === '}') {
      repBraceCount--;
      if (repBraceCount === 0 && repInFunction) {
        repEnd = i;
        break;
      }
    }
...
const newRep = `export function GuidanceReports() {
  const [submissions, setSubmissions] = useState<any[]>([]);
...
content = content.substring(0, repStart) + newRep + content.substring(repEnd + 1);
